...
test('processes simple markdowns', function (t) {
    var actualMetamark = getMetamark('test/test-simple.md');
...
test('processes single-h2 markdown', function (t) {
    var actualMetamark = getMetamark('test/test-single-h2.md');
    var expectedToc = [{ title: 'Chapter One', url: '#chapter-one' }];
...
test('processes double-h2 markdown', function (t) {
    var actualMetamark = getMetamark('test/test-double-h2.md');
    var expectedToc = [
...
test('processes single-h2-single-h3 markdown', { only: true }, function (t) {
    var actualMetamark = getMetamark('test/test-single-h2-single-h3.md');
    var expectedToc = [
        {
            title: 'Chapter One',
            url: '#chapter-one',
...
function assertArrayEquality(a1, a2) {
    var is1 = Array.isArray(a1);
    var is2 = Array.isArray(a2);
    if (!is1 || !is2)
        assert.fail("".concat(i(a1), " or ").concat(i(a2), " is not an array"));
    var l1 = a1.length;
    var l2 = a2.length;
    if (l1 != l2)
        assert.fail("Unequal array length: ".concat(l1, " and ").concat(l2, " for ").concat(i(a1), " and ").concat(i(a2)));
